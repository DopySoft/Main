...
def resizeAndSave(billingWidget):
    #def function():       
        ...
        billingPath = destinationPath + "\\" + rollName
        deptPath = "\\".join(destinationPath.split("\\")[:-1]) + "\\Departments"
        outsiPath = "\\".join(destinationPath.split("\\")[:-1]) + "\\Outsi"
        ...
        for snap in roll.snapList:
            ...
            for entry in idList:   
                ...
                for copyNo in range(numCopies):
                    try:
                        ...
                        if snapExtension:
                            print("has extension")
                            outfile = open(billingPath+"\\"+"{}_{}_{}_{}.{}".format(roomDetails,snapName,copyNo + 1,id,snapExtension), 'wb')
            ...
            if snap.dept:
                ...
                try:
                    os.mkdir(deptPath)
                    os.mkdir(deptPath+"\\{}".format(snap.dept))
                except:
                    try:
                        os.mkdir(deptPath+"\\{}".format(snap.dept))
                    except:
            ...
            if snap.outsi:
                ...
                row+=";"
                try:
                    csvFile = open(outsiPath + "\\outsi.csv", 'a+')
